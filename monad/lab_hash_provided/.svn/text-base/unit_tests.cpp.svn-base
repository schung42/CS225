...
#if MONAD_SPLIT
UNIT_TEST(testRemoveEASY, 1, 1, 5000){
...
#if MONAD_SPLIT
UNIT_TEST(testRemoveHard, 1, 1, 5000){
...
UNIT_TEST(testResize, 1, 1, 5000){
	...
	if( hashTable.find("further") != 9 )
		FAIL("Didn't copy over existing lists properly on resize");
	if( hashTable.find("decided") != 12 )
		FAIL("Didn't re-hash keys properly");
}
#endif

#if MONAD_SPLIT
UNIT_TEST(testInsertEasy, 1, 1, 5000){
	TextFile infile( "textEasy.txt" );
...
#if MONAD_SPLIT
UNIT_TEST(testInsertHard, 1, 1, 5000){
	TextFile infile( "textHard.txt" );
	LPHashTable<string, int> hashTable(32);
    int val = 0;
	while( infile.good() ) {
...
#if MONAD_SPLIT
UNIT_TEST(testAnagramsSC, 1, 1, 5000){
    testAnagrams<SCHashTable>();
}
#endif

#if MONAD_SPLIT
UNIT_TEST(testAnagramsLP, 1, 1, 5000){
...
#if MONAD_SPLIT
UNIT_TEST(testWordCountSC, 1, 1, 5000){
    testWordCount<SCHashTable>();
}
#endif

#if MONAD_SPLIT
UNIT_TEST(testWordCountLP, 1, 1, 5000){
...
UNIT_TEST(testLogfileParse, 1, 1, 5000){
