...
int Game::delete_user(const string& username)
{
    ...
    if(user != users.end())
    {
        // if they exist and they're online, return fail
        if(user->second.status == User::ONLINE)
            return FAIL;
        // otherwise delete them and return success
        else
        {
    ...
    // if they didn't exist, return invalid
...
int Game::logout_user(const string& username)
{
    ...
    if(user != users.end())
    {
        // make sure they're not in a game
        if(users[username].status != User::IN_GAME)
        {
            // if not, log them out and return success
            user->second.status = User::OFFLINE;
            return SUCCESS;
        }
        // otherwise return in match
        else
...
unsigned long Game::make_match(const string& party_id)
{
    ...
    auto party_it = parties.find(party_id);
    if(party_it != parties.end())
    {
        // make sure no one is in a match
        for(string& username : party_it->second.members)
        {
            if(users[username].match_id != 0)
                return NULL_MATCH_ID;
        }
        
        // create the match, and try to add the party
        Match* matched = new Match;
        int check = matched->add_party(party_it->second);
        
        // check for faliure
        if(check == -1)
            return NULL_MATCH_ID;
        
        // make a pair and insert it to the matches map
        std::pair<unsigned long, Match> match_pair;
        match_pair = make_pair(next_match_id, *matched);
        matches.insert(matches.end(), match_pair);
        
        next_match_id += 1;
        
        // less one because we incremented it
...
string Game::make_party(const vector< string >& usernames)
{
    ...
    for(auto username : usernames)
    {
        ...
        if((user_it == users.end()) || (users[username].party_id != ""))
            return "";
    }
    ...
    // put all the users in that party
    for(auto username : usernames)
    {
        auto user_it = users.find(username);
        users[username].party_id = party_name;
    }
    
    // construct a pair for the parties map
    std::pair<string, Party> party_pair;
    party_pair = make_pair(party_name, *group);
    
    // insert it into the map and return the id value
    auto party_it = parties.insert(parties.end(), party_pair);
    return party_name;
}
