...
void call_sketchify(const string & imageFile)
{
	...
	char sketchify_argv0[] = "./sketchify";
	char * sketchify_argv[] = { sketchify_argv0 };
	proxy_test::main((sizeof sketchify_argv) / (sizeof sketchify_argv[0]), sketchify_argv);
}
...
UNIT_TEST(similar_sketch, 0, 30, 1000)
{
	...
	for (int y = 0; y < soln_out01.height(); y++)
	{
		for (int x = 0; x < soln_out01.width(); x++)
		{
...
UNIT_TEST(complete_in_01_bmp, 0, 20, 1000)
{
	call_sketchify("in_01.png");
	ASSERT(exists("out.png"));
...
UNIT_TEST(complete_in_02_bmp, 0, 20, 1000)
{
