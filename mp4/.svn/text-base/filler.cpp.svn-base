...
animation filler::dfs::fill( PNG & img, int x, int y, 
     ...
     */
   animation gif = filler::fill<Stack>(img,x,y,fillColor,tolerance,frameFreq);
   return gif;
}
...
animation filler::bfs::fill( PNG & img, int x, int y, 
     ...
     *  indicating the ordering structure to be used in the fill.
     */
    animation gif = filler::fill<Queue>(img,x,y,fillColor,tolerance,frameFreq);
    return gif;
}
...
animation filler::fill( PNG & img, int x, int y,
     ...
     *
     *        For example, if frameFreq is, then after the 4th pixel has
    ...
    bool unprocessedElement[img.width()][img.height()];
    RGBAPixel initial = *img(x,y);
    
    for(int i = 0; i< img.width() ; i++)
    {
    	for(int j = 0; j<img.height(); j++)
    	{
    		unprocessedElement[i][j] = true;
    	}
    }
    ...
    while(!xcoord.isEmpty() || !ycoord.isEmpty())
    {
    	...
    	RGBAPixel* current = img(xcurr,ycurr);
    	if(unprocessedElement[xcurr][ycurr])
    	{
    		int tol = ((current->red - initial.red) * (current->red - initial.red)) + 
    		((current->green - initial.green) * (current->green - initial.green)) + 
    		((current->blue - initial.blue) * (current->blue - initial.blue));
    		
    		if(tol <= tolerance)
    		{
    			unprocessedElement[xcurr][ycurr] = false;
    			*img(xcurr, ycurr) = fillColor(xcurr,ycurr);
    			if(xcurr+1 < img.width()) //right
    			{
    			xcoord.add(xcurr +1);
    			ycoord.add(ycurr);
    			}
    			if(ycurr +1 < img.height()) //up
    			{
    			xcoord.add(xcurr);
    			ycoord.add(ycurr+1);
    			}
