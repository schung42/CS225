...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    while(subroot != NULL)
    {
        ...
        if(subroot->elements[retVal].key == key)
        {
            return subroot->elements[retVal].key;
        }
        if(subroot->is_leaf)
        {
            if(key != subRoot->elements[retVal].key)
            {
                return V();
            }
        }
        return find(subRoot->children[retVal], key);
        }
    }
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    //create new node which is going to store order - 1 keys of parent
    BTreeNode* newChild = new BTreeNode(parent->children->order, parent->is_leaf);
    newChild->children.size()= order -1;

    //copy the last order-1 keys of parent into newChild
    for(int j = 0; j< (order-1); j++)
    {
        newChild->children[j] = parent->children[j+order];
    }

    //copy the last order children of parent into newChild
    ...
    //reduce number of keys in parent
    parent->children.size() = order - 1;

    //since this node is goin gto have a new child, create space of new child
    ...
    //a key of parent will move to this node. find location of new key and move all greater keys once space ahead
